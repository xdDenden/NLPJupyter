"""
main.py
train → infer → benchmark → HTML report + CSV

we tried to make it as simple as possible to run and get a result thats visual and contains all necessary data

To swap the backbone model, change BASE_MODEL in config.py
"""
...
# DATA PREPARATION
# Align BIO labels with tokenized inputs, handling subword splits
...
def extract_dosages(text):
    ...
    return [
        ...
        # this function just uses regex as we know they get expressed as numbers not text
    ]


# we had this function mostly generated by AI (claude)
...
# predicting with our custom model is a bit more involved as we have to handle the CRF decoding and then convert the predicted tag ids back into entity spans
# ended up working out with no issues so far
...
def predict_with_pipeline(text, pipe):
    """Inference via a standard HuggingFace pipeline"""
    return [
...
# Calculate precision, recall, and F1 from TP/FP/FN counts for a given label
# definitely generated by AI (claude) but it works and is pretty straightforward so no complaints
def _p_r_f1(s):
...
#had some help from AI here too as we are not very html familiar
...
def run_evaluation(model_path, baseline_path, train_first=False):
    ...
    # Custom sentences we use on every run to compare the models on fixed examples
    ...
    # benchmarking the dataset on both models
    ...
    # finally generating the HTML and CSV reports with all the stats and visualisations, and saving them to the report directory
    print("\nGenerating reports …")
